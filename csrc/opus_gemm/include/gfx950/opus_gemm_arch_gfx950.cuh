...
//
// gfx950 a16w16 selector and strict kid dispatch.
#pragma once

#include "../opus_gemm_arch.cuh"
#include "../opus_gemm_common.cuh"
#include "../opus_gemm_utils.cuh"
#include "opus_gemm_heuristic_dispatch_gfx950.cuh"
...
inline const OpusA16W16WorkspaceTuneEntry* workspace_entry(int kid)
{
    static constexpr std::array<
        ...
        GENERATE_A16W16_WORKSPACE_TUNE_LOOKUP_GFX950_SIZE>
        kWorkspace = {{GENERATE_A16W16_WORKSPACE_TUNE_LOOKUP_GFX950}};
...
inline void check_shape_4g(int M, int N, int K, size_t c_element_size)
{
    ...
    AITER_CHECK(a_bytes <= U32_MAX_BYTES && b_bytes <= U32_MAX_BYTES &&
                ...
                "opus gfx950 a16w16 heuristic refuses >4 GiB shape (M=",
                M,
                " N=",
                N,
                " K=",
                K,
                "): use a tuned 4g_safe kid");
}
} // namespace opus_gfx950_detail
...
inline OpusA16W16Kernel opus_a16w16_tune_dispatch_gfx950(int id);

template <>
inline OpusA16W16Kernel opus_a16w16_tune_dispatch_gfx950<bf16_t>(int id)
{
    using namespace opus_gfx950_detail;
    static constexpr std::array<
        OpusA16W16TuneEntry, GENERATE_A16W16_TUNE_LOOKUP_GFX950_BF16_SIZE>
        kTune = {{GENERATE_A16W16_TUNE_LOOKUP_GFX950_BF16(bf16_t)}};
    const auto* entry = find_kid(kTune, id);
    AITER_CHECK(entry != nullptr,
                "Kernel id ",
                id,
                " not found in gfx950 a16w16 bf16 non-workspace table");
    return entry->func;
}

template <>
inline OpusA16W16Kernel opus_a16w16_tune_dispatch_gfx950<fp32_t>(int id)
{
    using namespace opus_gfx950_detail;
    static constexpr std::array<
        OpusA16W16TuneEntry, GENERATE_A16W16_TUNE_LOOKUP_GFX950_FP32_SIZE>
        kTune = {{GENERATE_A16W16_TUNE_LOOKUP_GFX950_FP32(fp32_t)}};
    const auto* entry = find_kid(kTune, id);
    AITER_CHECK(entry != nullptr,
                "Kernel id ",
                id,
                " not found in gfx950 a16w16 fp32 non-workspace table");
    return entry->func;
}
...
opus_a16w16_workspace_dispatch_gfx950(int id)
{
    ...
    AITER_CHECK(entry != nullptr,
                "Kernel id ",
                id,
                " not found in gfx950 a16w16 workspace table");
    return entry->func;
}

// Legacy `(M,N,K)` policy probe.  It selects an integer kid only; launch is
// deliberately left to the ABI-correct strict dispatch above.
template <typename CDataType>
inline int opus_select_a16w16_kid_gfx950(
    int M, int N, int K, int batch, bool has_bias = false);

template <>
inline int opus_select_a16w16_kid_gfx950<bf16_t>(
    int M, int N, int K, int batch, bool has_bias)
{
    using namespace opus_gfx950_detail;
    static constexpr std::array<
        OpusA16W16RuntimeEntry, GENERATE_OPUS_LOOKUP_TABLE_GFX950_BF16_SIZE>
        kLookup = {{GENERATE_OPUS_LOOKUP_TABLE_GFX950_BF16}};
    const int tuned_kid = find_shape_kid(kLookup, M, N, K);
    if(tuned_kid >= 0) return tuned_kid;
    (void)batch;
    check_shape_4g(M, N, K, sizeof(bf16_t));
    return opus_a16w16_heuristic_kid_gfx950(M, N, K, has_bias);
}

template <>
inline int opus_select_a16w16_kid_gfx950<fp32_t>(
    int M, int N, int K, int batch, bool has_bias)
{
    using namespace opus_gfx950_detail;
    static constexpr std::array<
        OpusA16W16RuntimeEntry, GENERATE_OPUS_LOOKUP_TABLE_GFX950_FP32_SIZE>
        kLookup = {{GENERATE_OPUS_LOOKUP_TABLE_GFX950_FP32}};
    const int tuned_kid = find_shape_kid(kLookup, M, N, K);
    if(tuned_kid >= 0) return tuned_kid;
    (void)batch;
